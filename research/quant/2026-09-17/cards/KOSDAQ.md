...
- **Reason code(s):** RECLAIM_UNCONFIRMED, INSUFFICIENT_HISTORY, INADEQUATE_LIQUIDITY, NO_CATALYST, THESIS_NOT_FALSIFIABLE, NO_MISPRICING
...
Session vs peers is ordinary on this one print. This name is the operational benchmark for its group, not a mispricing candidate.

## Evidence

| claim | source | timestamp | capture | evidence_confidence | provenance |
| --- | --- | --- | --- | --- | --- |
| session print last=820.73 chg=4.75 chg%=+0.58% | principal_watchlist_screenshot | 2026-09-17T02:42:00Z | IMP-001 screenshots; clock_on_image=false | 0.40 | fixture/watchlist snapshot; not fabricated |

evidence_confidence is data reliability (0–1), not a board verdict.

## Relative performance and structure

Session vs last print: +0.58% (one session; not a structure regime). Bullish reclaim is unconfirmed: needs prior breakdown level, reclaim of that level, and hold ≥ 2 sessions. A one-day bounce is not a reclaim. No native history window in this snapshot — trend/range/vol path unknown. Session vs peer median (kr_complex peers SAMSUNG): +0.58% vs median -1.39% (gap +1.97 pp). This is a relative-value observation, not ARBITRAGE.
