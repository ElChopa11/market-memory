...
- **Promotion checklist:** fresh_attributable_data=yes, defined_benchmark_peers=no, specific_anomaly=no, overlooked_reason=no, catalyst_or_trigger=no, single_falsifiable_invalidation=no, acceptable_liquidity=no, no_unaddressed_duplicate_beta_or_dq=no, independent_skeptic_review_required=yes

## What is objectively unusual?

Nothing objectively unusual in this snapshot.

## Evidence

| claim | source | timestamp | capture | evidence_confidence | provenance |
| --- | --- | --- | --- | --- | --- |
| session print last=102.19 chg=-0.24 chg%=-0.23% | principal_watchlist_screenshot | 2026-09-17T02:42:00Z | IMP-001 screenshots; clock_on_image=false | 0.40 | fixture/watchlist snapshot; not fabricated |

evidence_confidence is data reliability (0–1), not a board verdict.

## Relative performance and structure

Session vs last print: -0.23% (one session; not a structure regime). Bullish reclaim is unconfirmed: needs prior breakdown level, reclaim of that level, and hold ≥ 2 sessions. A one-day bounce is not a reclaim. No native history window in this snapshot — trend/range/vol path unknown.
