# Skeptic — watchlist / active-call EXPECTATIONS methodology scorecard
...
| ETH | active | **FAIL** |
| NVDA | active | **PASS** |
| AVGO | active | **INCONCLUSIVE** |
| MSFT | active | **INCONCLUSIVE** |
| META | active | **INCONCLUSIVE** |
| JPM | active | **FAIL** |
| XLF | active | **FAIL** |
| XOM | active | **PASS** |
| UNI | watch | **FAIL** |
...
### ETH — **FAIL**
1. **Stated expectation** — Call: conditional RV vs BTC. Expected path: 1–5s tracks BTC with possible funding skew; cycle IF = ETH/BTC outperformance if ETF + L2 gates clear (call-cards §2). QUANT: raw vs BTC **NOT α**; OI/mark ULIDs missing; HL omitted/429.
...
### NVDA — **PASS**
1. **Stated expectation** — Call: conditional; crowded; needs mispricing angle not “AI demand exists.” Expected path: 1–5s high-beta / crowded unwind risk; cycle IF upside only with documented mispricing vs consensus (call-cards §5). QUANT: crowded; macro ≠ name edge.
...
### AVGO — **INCONCLUSIVE**
1. **Stated expectation** — Call: conditional satellite to NVDA; independence of evidence required. Expected path: 1–5s high corr to NVDA/SMH; cycle IF satellite add only if AI line-item / customer cohort show **independent** cash-conversion (call-cards §6). QUANT: NVDA–AVGO 0.46 ≠ two-name license.
...
### MSFT — **INCONCLUSIVE**
1. **Stated expectation** — Call: conditional / earnings-gated. Expected path: 1–5s quality drift; cycle IF multiple holds if Azure growth + capex ROI confirm (call-cards §8). QUANT: large raw vs-SPY 63d is **not** residual α; RV20≪RV60 ≠ regime ID.
...
### META — **INCONCLUSIVE**
1. **Stated expectation** — Call: conditional / earnings-gated ads+AI ROI. Expected path: 1–5s high-beta to ad/AI headlines; cycle IF re-rate only if ad growth + infra ROI beat embedded consensus (call-cards §9). QUANT: META asof **2026-09-15** lag; raw vs SPY **not contemporaneous**.
...
### JPM — **FAIL**
1. **Stated expectation** — Call: conditional; higher-for-longer NII OK but **likely priced** post-FOMC/SEP. Expected path: 1–5s may already reflect SEP; cycle IF further upside only if NII guide beats SEP-implied path (call-cards §10). QUANT: no allocation from FOMC; crowded narrative.
...
### XLF — **FAIL**
1. **Stated expectation** — Call: conditional/watch diversifier to JPM. Expected path: 1–5s tracks JPM/financials; cycle IF diversifier value if non-JPM banks/capital markets add breadth (call-cards §11). QUANT: JPM–XLF 0.73 is sample description not a pair trade.
...
### XOM — **PASS**
1. **Stated expectation** — Call: conditional / crude-gated; kill “Fed hike → XOM up.” Expected path: 1–5s tracks crude strip/inventory; cycle IF FCF/buyback if strip holds (call-cards §12). QUANT: hike ≠ XOM upside; negative corr ≠ energy sleeve.
...
### UNI — **FAIL**
1. **Stated expectation** — Call: conditional / event-gated fee-switch. Expected path: 1–5s drift no edge; cycle IF binary re-rate if dated fee-switch lands (call-cards §3). Universe: watch-only (`universe.yaml`). QUANT: extreme 30d/90d bounce footnoted not thesis; HL appendix only.
...
### AAVE — **FAIL**
1. **Stated expectation** — Call: conditional crypto credit under higher-for-longer; micro-size. Expected path: 1–5s illiquid drift; cycle IF utilization/revenue rise (call-cards §4). Watch-only in universe.yaml.
...
### SMH — **FAIL**
1. **Stated expectation** — Call: conditional/watch basket; double-count disclosed. Expected path: 1–5s tracks AI-semi; cycle IF basket outperforms if buildout broadens beyond NVDA (call-cards §7). Watch-only.
...
### IBIT — **INCONCLUSIVE**
1. **Stated expectation** — Principal greenlit **IBIT watch-only** as BTC equity-proxy / TradFi wrapper adjacent to RQ-A / BTC (sidebar in killed `INTENT-20260917-memory-semi.md` only; **not** in `universe.yaml` active/watch lists on main; **no** call-card section).
