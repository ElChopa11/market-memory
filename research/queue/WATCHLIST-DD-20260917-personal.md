...
2. **MAKE 2 · WATCH 28 · CUT 33** (underlyings only). Pair verdicts: **1 valid monitor / 6 invalid or double-count**.
...
5. **Rejection bias holds:** must-cuts (HYPE SOL XRP NEAR) + memory-semi (MU SNDK SKHYNIX) + memes (DOGE PURR CASHCAT SPX6900 …) = **CUT**; ETH/JPM/XLF-style FAIL expressions → see appendix (not MAKE).
...
| NVDA | Base | **MAKE** |
| GOOG | Base | **WATCH** |
| AMD | Base | **WATCH** (AI overlap risk) |
...
| BB | Base | **CUT** (thin) |
| GLXY | Base | **WATCH** (BTC equity-proxy cluster) |
...
| STRC | Base | **CUT** (thin) |
| CRCL | Base | **WATCH** (stablecoin/issuer narrative — thin until filings) |
...
| AI / mega-cap tech | NVDA, AMD, GOOG, NOW, QQQ, NQ | NVDA solid; others overlap or thin |
| Crypto equity proxies | IBIT, MSTR*, GLXY, CRCL? | IBIT = valid watch wrapper; MSTR = CUT double-count |
...
| **GLXY** | Crypto equity / trading proxy | **Weak WATCH** | Soft BTC/crypto beta; not a lab active. |
...
| **GOLD / SILVER (+ BTC/GOLD, BTC/SLV)** | Real-asset / crypto-relative | **Context WATCH**; gold *call* learning = must-cut **GLD** | Metals OK as dashboard; do not revive GLD active expectation. |
...
#### NVDA — **MAKE**
Locked yaml `active_calls.equities` includes NVDA. QUANT: last **213.90** asof **2026-09-16**; RV20/RV60 **45.5% / 40.4%** √252; MDD **−20.2%**; vs SPY 1m/3m/YTD **−2.4% / +2.5% / +2.6%** (simple-diff **NOT α**); ADV 5d ≈ **102.3M** sh / **$21.70B**. Scorecard **PASS** (crowded honesty + mispricing gate empty). Corr NVDA–AVGO **0.46** (60d, 2026-05-28→2026-09-15) = co-movement description, not a multi-name license. **MAKE** as AI-infra primary; do not auto-MAKE AMD/SMH/memory from this.

### 4.2 WATCH (28) — monitors / demotions / context
...
#### AMD — **WATCH**
AI/semi overlap with NVDA primary. Without independent residual vs NVDA, risk of double-count — keep WATCH ceiling, prefer CUT if forced to binary; retained WATCH for liquidity/dashboard honesty.
...
#### CRCL — **WATCH**
Circle / stablecoin-issuer narrative; thin until primary filings and flow prints. Not MAKE.

#### GLXY — **WATCH**
Soft crypto equity beta; secondary to IBIT/BTC.

#### QQQ, NQ, SPX — **WATCH**
Index/risk dashboards. Shortlist explicitly deferred SPY/QQQ as calls.

#### CL, NG, COPPER, GOLD, SILVER — **WATCH**
Commodity path context (esp. CL for XOM crude gate family). Not equity substitutes. GLD *call* remains must-cut learning — do not revive via GOLD metal.
...
| **BTC/SLV** | **Weak / mostly redundant** | Silver vs BTC adds little vs BTC/GOLD; optional CUT of pair, metals already WATCH. |
...
**Pair score:** valid monitor **1** (BTC/GOLD) · invalid/double-count **6**.
...
|---|---:|---|
| **MAKE** | **2** | BTC, NVDA |
| **WATCH** | **28** | ETH, UNI, BNB, IBIT, GOOG, AMD, NOW, CRCL, GLXY, QQQ, NQ, SPX, CL, NG, COPPER, GOLD, SILVER, DXY, TNX, VIX, KOSDAQ, BTC.D, USDT.D, TOTAL, TOTAL2, TOTAL3, OTHERSBTC, STABLE.C |
| **CUT** | **33** | must-cuts HYPE SOL XRP NEAR · killed MU SNDK SKHYNIX · memes/lottery · thin crypto/equities · IWM · UX |
| **Pairs** | 7 | 1 valid monitor · 6 invalid/double-count |
| **Distinct underlyings** | **63** | |

Exact lists in §1 disposition columns (MAKE 2 + WATCH 28 + CUT 33 = 63).
