...
- No sizing.
- No execution, no order intent, no signing, no wallet path.
...
## HL structure (never invent missing prints)

Public Hyperliquid `/info` only. Missing metrics stay `unavailable`. Capture clock is `as_of_knowledge` (lockstep with `ingested_at`); never `published_at` / `market_time`.

| metric | value | source | as_of_knowledge | evidence_confidence | provenance |
| --- | --- | --- | --- | --- | --- |
| funding | unavailable |  |  |  |  |
| open interest | unavailable |  |  |  |  |
| basis mark minus oracle | unavailable |  |  |  |  |
