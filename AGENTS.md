...
**Phase 6a (IMP-014):** Desk mesh in `packages/desks` (`mm_desks`) + Market Memory bus (`mm_memory.notify`): cadence metadata, DeskOutput envelope + `content_hash` idempotency, Postgres `LISTEN/NOTIFY` (no Redis). Channels `desk.<slug>.output` / `alert`, `coord.assemble`, `dq.event`. Envelopes persist; NOTIFY is ids only. Coord worker stub assembles from Memory; missing desk → `FAILED` + `error_class`. CLI `lab mesh dry --fixture --no-db` and `lab desk run --no-send`. **No** live trading, no wallet code, no `hl_trade` / signing. Do not implement Phase 6b flow/macro/regime, per-desk Telegram fan-out, listings/IPO, scorecards, decay/prompt versioning, risk *service*, execution service, or live paths in this phase.

Desk operating model (private research lab, not a fund): [ops/desk-charters.md](ops/desk-charters.md), [ops/decision-rights.md](ops/decision-rights.md), [ops/improvement-queue.md](ops/improvement-queue.md), [docs/runbooks/desks.md](docs/runbooks/desks.md), [docs/runbooks/telegram.md](docs/runbooks/telegram.md). Hive roles in this file remain the permission constitution; desks are how work is assigned. **No desk overrides the Principal.** Execution & Fund Ops is future-only. Delivery (Telegram) is Phase 5e. Flow/macro/regime is **Phase 6b** (IMP-015, parked).
...
Before claiming a code change is done: no secrets in the diff, tests/CI green, Phase 6a scope respected (desk mesh + PG NOTIFY bus + `--no-send` packs; Telegram delivery + desk runners + backtest harness + paper ledger remain; no execution/signing, no risk service, no live path, no Redis, no 6b flow/macro), live still hard-gated, `what_did_we_know` keyed off `as_of_knowledge` (lockstep with `ingested_at`; never `published_at` / `market_time`), backtests keyed off `available_at`, rejected theses still queryable, paper open still requires invalidation + max loss. Import-boundary check (`scripts/check_import_boundaries.py`) is green. Pytest never hits the live Telegram API.
