...
**Phase 5c (IMP-011):** Quant factor library in `packages/quant` (`mm_quant`): momentum, realised vol, ADX-style trend, z-scores, funding/basis carry (consumes Phase 5b HL structure fields when present), relative strength, breadth, correlation, beta; YAML regime thresholds; stat + intent-level budget-fraction helpers; factor QuantCard. Phase 4 capabilities (fixture backtest + paper/shadow ledger), Phase 5a desk boundaries, and Phase 5b Polygon + HL structure ingest remain. **No** Telegram send, **no** desk runners, **no** live trading, no wallet code, no `hl_trade` / signing. Do not implement the deterministic risk *service*, execution service, or live paths in this phase.
...
Before claiming a code change is done: no secrets in the diff, tests/CI green, Phase 5c scope respected (quant factor library; backtest harness + paper ledger remain; no execution/signing, no risk service, no live path, no Telegram/runners), live still hard-gated, `what_did_we_know` keyed off `as_of_knowledge` (lockstep with `ingested_at`; never `published_at` / `market_time`), backtests keyed off `available_at`, rejected theses still queryable, paper open still requires invalidation + max loss. Import-boundary check (`scripts/check_import_boundaries.py`) is green.
