...
**Phase 6c (IMP-016):** Per-desk Telegram fan-out + presentation + chart desk + inbound in `packages/delivery`. Hive PLAYBOOK ladder in `packages/desks` (`mm_desks.playbook`): `DAILY_BIAS` … `STATE_CARD` sharing `run_id` + `content_hash`. Quant-owned trade math in `mm_quant.trade_math` (inherited hash; mismatch = failed run). Addendum 6c-0: LLM is WRITER/CRITIC only; `config/llm/budgets.yaml` enforced at the client; versioned `config/prompts/`; numeric/entity/render locks; no-setup fixture = **zero LLM calls**. CLI `lab playbook run --fixture --no-send` and `lab deliver fanout --no-send`. **No** live trading, no wallet code, no `hl_trade` / signing, **no** listings/IPO (6d), **no** live LLM HTTP. Do not implement scorecards automation, strategy decay-watch remainder, risk *service*, execution service, or live paths in this phase.

Desk operating model (private research lab, not a fund): [ops/desk-charters.md](ops/desk-charters.md), [ops/decision-rights.md](ops/decision-rights.md), [ops/improvement-queue.md](ops/improvement-queue.md), [docs/runbooks/desks.md](docs/runbooks/desks.md), [docs/runbooks/flow-desk.md](docs/runbooks/flow-desk.md), [docs/runbooks/macro-desk.md](docs/runbooks/macro-desk.md), [docs/runbooks/telegram.md](docs/runbooks/telegram.md), [docs/runbooks/llm-budget.md](docs/runbooks/llm-budget.md), [docs/playbook.md](docs/playbook.md). Hive roles in this file remain the permission constitution; desks are how work is assigned. **No desk overrides the Principal.** Execution & Fund Ops is future-only. Delivery (Telegram) is Phase 5e; per-desk fan-out + PLAYBOOK is **Phase 6c** (IMP-016). Listings/IPO is **Phase 6d** (IMP-017, parked).

## Non-negotiables

1. **Research cannot access trading credentials.** No API wallet keys, no agent-wallet secrets, no treasury material, no `.env` live keys, no vault paths for execution. Research tools are Market Memory read + artifact write. `packages/research_kit`, `packages/desks`, `packages/quant`, `packages/delivery`, `packages/flow`, and `packages/macro` must not import `mm_execution`, ingest private keys, or grow a signing surface.
...
6. Do not commit secrets. Do not add Hyperliquid signing, order submission, or exchange-module usage. Ingest is public `/info` only. Intel (Tier 2) must not import opine packages (`mm_research_kit`, `mm_desks`, `mm_quant`, `mm_delivery`, `mm_flow`, `mm_macro`).
...
| **Coordinator** | 1 | Queue work; run DoD / `check-lifecycle`; `lab thesis` / `lab skeptic` / `lab backtest` / `lab paper` / `lab desk run` / `lab mesh` / `lab deliver` / `lab playbook`; decompose tasks; open PRs for research; run `lab migrate` / `lab ingest` | Hold or inject trading credentials; promote to live; waive skeptic; self-approve | None for trading |
...
| **Delivery** | 5e (Telegram) | Desk-pack send via Bot API; dry-run default | Alert without thresholds; trading inbound; Phase 6 mesh | `TELEGRAM_*` env on the bot box only |
...
Before claiming a code change is done: no secrets in the diff, tests/CI green, Phase 6c scope respected (per-desk Telegram fan-out + presentation + chart desk + inbound + PLAYBOOK ladder + Quant-owned trade math + 6c-0 token budgets/grounding/versioned prompts on the 6a PG NOTIFY mesh; no execution/signing, no risk service, no live path, no Redis, no 6d listings, no live LLM HTTP), live still hard-gated, `what_did_we_know` keyed off `as_of_knowledge` (lockstep with `ingested_at`; never `published_at` / `market_time`), backtests keyed off `available_at`, rejected theses still queryable, paper open still requires invalidation + max loss. Import-boundary check (`scripts/check_import_boundaries.py`) is green. Pytest never hits the live Telegram API. A no-setup fixture day makes zero LLM calls.
