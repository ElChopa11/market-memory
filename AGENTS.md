...
**Phase 6b (IMP-015):** Flow/liquidity (`packages/flow`, `mm_flow`) and macro regime (`packages/macro`, `mm_macro`) desk runners in `packages/desks`. Liquidity verdict `OK|THIN|UNTRADEABLE_AT_SIZE`. Envelope `regime` filled from `config/macro/regimes.yaml` (tag + confidence + two driving inputs) when the macro run succeeds. `EVENT_RISK` within N minutes of high-impact calendar events. Risk YAML stubs (no LLM). Publish via the Phase 6a PG LISTEN/NOTIFY mesh (no Redis). Coord still assembles if flow or macro fails. CLI `lab desk run --desk flow|macro|--all --fixture --no-send` and `lab mesh dry`. **No** live trading, no wallet code, no `hl_trade` / signing, **no** per-desk Telegram fan-out (6c). Do not implement listings/IPO, scorecards, decay/prompt versioning, risk *service*, execution service, or live paths in this phase.

Desk operating model (private research lab, not a fund): [ops/desk-charters.md](ops/desk-charters.md), [ops/decision-rights.md](ops/decision-rights.md), [ops/improvement-queue.md](ops/improvement-queue.md), [docs/runbooks/desks.md](docs/runbooks/desks.md), [docs/runbooks/flow-desk.md](docs/runbooks/flow-desk.md), [docs/runbooks/macro-desk.md](docs/runbooks/macro-desk.md), [docs/runbooks/telegram.md](docs/runbooks/telegram.md). Hive roles in this file remain the permission constitution; desks are how work is assigned. **No desk overrides the Principal.** Execution & Fund Ops is future-only. Delivery (Telegram) is Phase 5e. Per-desk Telegram fan-out is **Phase 6c** (IMP-016, parked).
...
| **Coordinator** | 1 | Queue work; run DoD / `check-lifecycle`; `lab thesis` / `lab skeptic` / `lab backtest` / `lab paper` / `lab desk run` / `lab mesh` / `lab deliver`; decompose tasks; open PRs for research; run `lab migrate` / `lab ingest` | Hold or inject trading credentials; promote to live; waive skeptic; self-approve | None for trading |
...
Before claiming a code change is done: no secrets in the diff, tests/CI green, Phase 6b scope respected (flow/macro packages + desk runners + real envelope regime tag + EVENT_RISK + Risk YAML stubs on the 6a PG NOTIFY mesh; no execution/signing, no risk service, no live path, no Redis, no 6c Telegram fan-out), live still hard-gated, `what_did_we_know` keyed off `as_of_knowledge` (lockstep with `ingested_at`; never `published_at` / `market_time`), backtests keyed off `available_at`, rejected theses still queryable, paper open still requires invalidation + max loss. Import-boundary check (`scripts/check_import_boundaries.py`) is green. Pytest never hits the live Telegram API.
