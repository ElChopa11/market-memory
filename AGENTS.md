...
**IMP-046:** Hybrid Step 4. Hive clock → `lab` CLI writes a completion row (`run_id`, trigger, fire time, offset vs anchor, exit status) under `ops/reports/scheduler/completions/` so IMP-042 miss-check can observe the executed path. Failed CLI is a fire. **`--no-send` only.** Freeze holds until step 5. Do not rewrite Hive prompts. IMP-044/045 stay BACKLOG.

**IMP-043 DONE #71:** Hybrid Step 2. Delivery-only Telegram env file + full-state env preflight (`lab env preflight`). Hive is a clock; `lab` is the sole publisher. Token lives in `/home/box/agent-data/delivery/telegram.env` (not the Grok Secrets card). `MISSING` required → fail. Per-desk `NOT CONFIGURED` does not fail. `getChat` verifies the group id. **`--no-send` only.** Real send is Principal step 5. Isolation (IMP-044) and per-desk topics vs groups (IMP-045) are BACKLOG — do not build.
