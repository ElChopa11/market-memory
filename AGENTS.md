...
**IMP-042:** Ops scheduler miss detector. Closed window + no completion row → escalate (`lab schedule miss-check`). Heartbeat-on-fire is a log, not the check. SCHED-001 stays OPEN. CI uses a fixture clock. **No** live trading, no canary code, no auto-close.
