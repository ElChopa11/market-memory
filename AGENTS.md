...
**IMP-047:** Hybrid Step 5a. DM-only live path: `lab deliver test --to-principal-dm --i-mean-it` POSTs only to `TELEGRAM_CHAT_ID_PRINCIPAL_DM`. Missing that env → refuse. **Never** `TELEGRAM_CHAT_ID` (Hive group). Blanket group / desk pack `--send` stays `SEND_FROZEN`. Preflight still runs. `--routine-id` still stamps a completion row. Live acceptance is on-box only; unit tests may mock Telegram HTTP. IMP-048 weekly authoring CLI is BACKLOG — agents must not author weekly. IMP-049 Hybrid clock prompt canonical copies + server read-back is BACKLOG — do not build. Grok/Hive prompt bodies are server-kept and not diffable; Step 3 acceptance is Principal panel read-back of saved prompt text, not write-API success. Do **not** touch the miss detector (#68 / miss-check / IMP-042).
