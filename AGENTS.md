...
**IMP-024:** Intel SEC EDGAR adapter (`data.sec.gov` + Archives). Typed filing / lockup / 8-K 2.02 observations. `file_date` is not the knowledge clock. CBRS+SPCX lockup from stored 424B4 (not flat 180d). `licence_verdict: ok_gov`. No key. 403 → unavailable. Treasury.gov is IMP-035.

**IMP-034 DONE #60:** Principal ticker resolutions on the review list (SAMSUN→`KRX:005930`, KOSDA→`KRX:KQ11`) plus `licence_verdict` next to each ingest adapter. Standing rule: terms that prohibit redistribution → internal compute only; never publish those values. Incident close: `open→eligible→closed(cite run_id)|retired`. `--no-db` = ELIGIBLE only. FRED full-stack path is **IMP-022 DONE #60** (incident stays OPEN until a cited persist `run_id`).
