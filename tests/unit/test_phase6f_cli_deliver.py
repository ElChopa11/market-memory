"""Phase 6f CLI: lab deliver decay --no-send. Never hits live Telegram."""
...
def test_lab_deliver_decay_no_send(tmp_path: Path, capsys) -> None:
    rc = main(
        [
            "deliver",
            "decay",
    ...
    assert payload["decay_content_hash"] == payload["content_hash"]
