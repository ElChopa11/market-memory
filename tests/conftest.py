...
def db_session(postgres_dsn: str):
    ...
    session.execute(
        text(
            "TRUNCATE desk_health, desk_envelope, paper_trade, research_run, brief, skeptic_review, thesis_evidence, thesis_status_event, thesis, observation_link, observation, raw_object, source RESTART IDENTITY CASCADE"
        )
    )
    session.commit()
    try:
