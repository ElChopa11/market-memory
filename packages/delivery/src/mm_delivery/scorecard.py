"""Ops-owned Telegram delivery of IMP-030 Quant pack scorecards.

Does not invent like-for-like scores. Publisher is Ops.
`mm_delivery` must not import `mm_desks` — CLI/tests pass the Quant canonical dict.
"""
...
from mm_common.naming import QUANT, SCORECARD
...
def deliver_scorecard(
    ...
    *,
    ...
    """Fan-out a Quant pack scorecard on the quant route + Ops mirror."""
    cfg = settings or load_telegram_settings(repo)
    product = cfg.product(SCORECARD)
    if product is None or not product.enabled:
        raise ValueError("scorecard delivery product is missing or disabled")
    if product.desk != QUANT:
        raise ValueError(f"scorecard delivery desk must be {QUANT!r}")
    markdown = present_scorecard(canonical)
    raw_as_of = as_of or canonical.get("as_of_knowledge")
    if raw_as_of is None:
        raise ValueError("scorecard delivery requires as_of_knowledge")
