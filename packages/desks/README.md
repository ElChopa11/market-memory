...
**Must not:** depend on the `mm_execution` module or signing; fetch Polygon (Intel/`mm_ingest`); submit orders; self-approve Skeptic/Risk; import Redis; call a live LLM as calculator/router. Telegram send is Ops-owned `lab deliver` / `lab deliver fanout` / `lab deliver watchlist` (IMP-013 / IMP-016 / IMP-021), not this package. PLAYBOOK: [../../docs/playbook.md](../../docs/playbook.md).
