"""Load versioned listings desk YAML. Thresholds live in config, not model weights."""
...
class ListingsConfig:
    ...
    kind: str = "listings_ipo_desk"
    desk: str = "Equities & Post-IPO Desk"
    n_min: int = 20
...
def load_listings_config(root: Path | None = None) -> ListingsConfig:
    ...
    return ListingsConfig(
        ...
        kind=str(raw.get("kind") or "listings_ipo_desk"),
        desk=str(raw.get("desk") or "Equities & Post-IPO Desk"),
        n_min=int(raw["n_min"]),
