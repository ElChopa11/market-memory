...
Live trading is **hard-gated** (`config/risk/environments/live.yaml` → `live_trading_enabled: false`). **Controlled universe is locked** (`config/universe.yaml`, Principal 2026-09-17). Ingest membership stays full: Hyperliquid **BTC, ETH, UNI, AAVE** perps; equities **NVDA, AVGO, SMH, MSFT, META, JPM, XLF, XOM, IBIT** are a Phase 3 briefing / future equity-feed watchlist, not HL. Survivors are **not equal priority** — **active calls** (thesis priority): BTC, ETH, NVDA, AVGO, MSFT, META, JPM, XLF, XOM; **watch-only** (no active call): UNI, AAVE, SMH, IBIT (Skeptic PR #14 / call cards PR #13; IBIT is Principal 2026-09-17 BTC equity proxy, BTC adjacency to RQ-20260917-A). Must-cuts (HYPE, SOL, XRP, ARB, NEAR, LINK, GLD, LLY) stay archived. Intent-level only — not orders. Ops timezone: **Australia/Sydney**; US session: **America/New_York** (DST via `zoneinfo`); all database timestamps are **UTC `timestamptz`**.
