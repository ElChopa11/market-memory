...
| **Dependencies** | IMP-033 DONE (#59). IMP-034 licence schema (#60). Principal env on the operator box. |
...
| **Lesson learned** | Merged to `main` (#60). Fixture `--no-db` is `ELIGIBLE` with value + `claim_hash`. Persist helper exists. `SRC-FRED-MISSING-ENV` stays OPEN until an operator cites a persist `run_id`. `licence_verdict: ok_gov`. |
...
| **Dependencies** | IMP-022 DONE (#60). Parked until EDGAR leaves the slot. |
...
| **Definition of done** | Queue: IMP-022 DONE (#60). This item the only implementation thread. Plan [plans/IMP-024-edgar.md](plans/IMP-024-edgar.md). Adapter + typed observations; PIT (`file_date` ≠ knowledge clock); CBRS+SPCX lockup confirmed from stored filing (`edgar-CBRS-424b4-20260513` / `edgar-SPCX-424b4-20260611` + `claim_hash`); missing/403 → unavailable + `error_class`; no `api.nasdaq.com` scrape; no Yahoo RSS. Tests + runbook + import-boundary. OPEN incidents untouched. |
| **Non-goals** | Street consensus; Finnhub estimates; listings as a sixth desk; Treasury.gov (IMP-035); IMP-023/025; `live.yaml` / signing / Redis. |
| **Dependencies** | IMP-022 DONE (#60). IMP-017 DONE (#54) listings sleeve. |
...
| **Dependencies** | IMP-022 DONE (#60). IMP-024 is EDGAR, not this item. |
| **Risk level** | Low. |
| **Status** | READY |
| **PR** | — |
| **Lesson learned** | *(fill at close)* |
...
| IMP-022 | Intel + Ops | Intel / Ops | DONE | [#60](https://github.com/ElChopa11/market-memory/pull/60) FRED full-stack path — `--no-db` ELIGIBLE; incident stays OPEN |
...
`IN_PROGRESS` count: **1** (IMP-024). IMP-000–IMP-022 and IMP-030–IMP-034 are `DONE`. FREE SOURCE PRIORITY order: IMP-022 DONE (#60) → IMP-024 (in progress) → IMP-035 READY → IMP-023 READY → IMP-036–038 BACKLOG. OPEN incidents: SCHED-001, BRIEF-TAG-20260918, SRC-STOOQ-404, SRC-FRED-MISSING-ENV (not closed; `--no-db` = ELIGIBLE only). Single implementation thread. Treasury.gov is IMP-035.
...
| SRC-FRED-MISSING-ENV | Ops | Ops (Principal for secrets) | OPEN | IMP-022 DONE (#60) shipped the path; `--no-db` = ELIGIBLE only; CLOSED needs cited run_id |
...
- IMP-032 merged as #57. IMP-033 merged as #59. IMP-034 + IMP-022 merged as #60 (KRX resolutions, `licence_verdict`, FRED ELIGIBLE path). This PR (IMP-024) wires SEC EDGAR. OPEN incidents untouched. SRC-STOOQ-404 stays OPEN. SRC-FRED-MISSING-ENV stays OPEN (`--no-db` = ELIGIBLE only). Locked universe unchanged. Paper only. Single-threaded: IMP-024 is the only `IN_PROGRESS`.
- Principal FREE SOURCE PRIORITY 2026-09-19 reorders source work (IMP-022 DONE / 024 IN_PROGRESS / 035 READY / 023 READY / 036 / 037 / 038). Paid items (IMP-027 CoinGlass Standard, IMP-028 paid Polygon SKUs, IMP-029 EODHD/Starter) stay Principal decision. Treasury.gov is IMP-035.
