...
Chart OPTION A (Principal 2026-09-20 FINAL; IMP-056) is **BACKLOG behind Monday 2026-09-21 unattended Sydney Morning**. This docs PR records the scope only. Do **not** retire `render_png` here. Paper only. No Telegram. No C-00x.
...
### IMP-056 — Chart OPTION A: librarian files TV snapshots (retire `render_png` after Monday fire)

| Field | Value |
|---|---|
| **ID** | IMP-056 |
| **Priority** | P2 — BACKLOG behind Monday 2026-09-21 unattended Sydney Morning |
| **Type** | Docs / chart product (queue-only this PR) |
| **Desk** | Research (chart sleeve) — librarian; not a sixth publishing desk |
| **Owner** | Research (files) / Ops (delivers via `lab deliver`) |
| **Problem** | CHART_ARTIFACT still computes levels and `render_png`. Retrieval framing (Chart browses / logs in / captures TradingView) and option B (server-side render such as mplfinance; automated TV) are the wrong shape. Principal 2026-09-20 FINAL supersedes both. |
| **Evidence** | Principal 2026-09-20 FINAL chart scope. Decision: **OPTION A**. `render_png` retires. No server-side rendering. No automated TV. Chart is librarian, not renderer, not retriever. IMP-016 PNG path is historical. IMP-051 Chart/TV webhook ban stays. Monday 2026-09-21 unattended Sydney Morning holds the clock; this item stays behind that fire. |
| **Proposed outcome** | Principal marks the chart in TradingView (entry/SL/TP via position tool; levels; measured moves) and snapshots (camera/Alt+S) to a hosted TV URL. Chart Desk does **not** retrieve, browse, log in, or capture. Chart receives the snapshot reference + structured levels and files them. Ops delivers via `lab deliver`. Chart never publishes. |
| **Definition of done** | **This PR (docs only):** queue + charter + house-lesson one-liners record OPTION A. No code. `render_png` stays until a later PR after Monday 2026-09-21 unattended Sydney Morning. **Later implementation (not now):** retire `render_png`; no mplfinance / server-side render; no automated or signed-in TV. CHART_ARTIFACT fields: `tv_snapshot_url` (as given, never constructed); `instrument` (exchange-qualified: HL:ZEC, HL:XMR, HL:ETH…); `timeframe` (as in chart header); `as_of` (snapshot timestamp from header); levels as structured fields not pixels (entry, stop, target(s), horizontals); `linked_thesis_id` (if Quant marked; else null); `content_hash`. Store under `briefs/`. Hand to Ops. Levels from markup only: Chart transcribes what the snapshot shows; does not read price action, infer, add, or adjust; ambiguous → record "ambiguous" and ask — never estimate. Watchlists: `config/watchlist/monitor.yaml` only. CASHCAT is tier BLOCKED: snapshot may be filed for reference but never carries an idea/size; Chart states the tier on every artifact. Paper only. No Telegram. No C-00x. Single IN_PROGRESS remains IMP-047. |
| **Non-goals** | Retiring `render_png` in this docs PR. Server-side rendering (mplfinance etc). Automated / signed-in TradingView. Browser automation. Screen capture. Telegram credential / webhook / direct publish. Trade view, direction, sizing, commentary. Making Chart a sixth publishing desk. Occupying IN_PROGRESS. Closing SCHED-001. Touching the miss detector. C-00x compute. Live trading. Universe / watchlist promotion. |
| **Dependencies** | Monday 2026-09-21 unattended Sydney Morning first. IMP-047 holds the implementation slot. IMP-051 Chart/TV webhook ban remains BACKLOG (complementary). IMP-016 PLAYBOOK ladder stays on main; this item supersedes the PNG/render path only. |
| **Risk level** | Low this PR (queued docs). High if Chart retrieves, renders, or publishes, or if CASHCAT snapshots carry an idea/size. |
| **Status** | BACKLOG |
| **PR** | this PR (docs/queue only; does not retire `render_png`) |
| **Lesson learned** | *(queued — Chart is librarian, not renderer, not retriever. OPTION A supersedes retrieval framing and option B. Do not retire `render_png` before Monday fire.)* |
...
| IMP-056 | Research (chart sleeve) | Research / Ops | BACKLOG | Principal 2026-09-20 FINAL OPTION A. Chart is librarian, not renderer, not retriever. `render_png` retires after Monday 2026-09-21 unattended Sydney Morning. Docs only this PR. Paper only. No Telegram. No C-00x. |

`IN_PROGRESS` count: **1** (IMP-047). IMP-046 is `DONE` (#72). IMP-043 is `DONE` (#71). IMP-042 is `DONE` (#68). IMP-041 is `DONE` (#67). IMP-040 is `DONE` (#66). IMP-039 candidate intake (#61) is `READY`; cards stay `INTAKE_ONLY`. IMP-000–IMP-022, IMP-024, IMP-030–IMP-034, and IMP-040–IMP-043 are `DONE`. IMP-044/045/048/049/050/051/052/053/054/055/056 stay BACKLOG. OPEN incidents: SCHED-001 (P0), BRIEF-TAG-20260918, SRC-STOOQ-404, SRC-FRED-MISSING-ENV (environment-propagation; prior CLOSED pack cited, not a key-absent close). Services/infrastructure OPEN: SRC-OBJECT-STORE (DOWN SERVICE, MinIO :9000). Delivery OPEN: TG-UNGATED-PRE-HYBRID (pre-Hybrid Telegram **ungated**). Single implementation thread. Hybrid Step 5a this PR; Hive group stays frozen. Prompt-body drift watch, per-channel `send_enabled`, and publisher inventory are queued, not built. IC Attack 5+A8 FIX ORDER is docs/report-only and does not occupy the slot.
...
| Crypto / equity / chart research cards and screens | Research (Investment Research) | Research. Chart sleeve is librarian (IMP-056 BACKLOG) — files TV snapshots; does not render, retrieve, or publish. |
...
Historical “active calls” language debt (membership keys) was a Gap; it is now **IMP-005 DONE** (#35). Do not reopen the key rename. Residual call-card *priority* language vs Quant SoT is **IMP-032 DONE** (#57). Canonical watchlist monitor.yaml is **IMP-033 DONE** (#59). Principal ticker resolutions + `licence_verdict` schema are **IMP-034 DONE** (#60). FRED full-stack is **IMP-022 DONE** (run_id `fred-fullstack-20260919-101938-aest`). SEC EDGAR is **IMP-024 DONE** (#63). Candidate strategy intake is **IMP-039 READY** (#61). Phase 1 unconditional base rates are **IMP-040 DONE** (#66). Desk knowledge base is **IMP-041 DONE** (#67). Scheduler miss detector is **IMP-042 DONE** (#68). Hybrid Step 2 is **IMP-043 DONE** (#71). Hybrid Step 4 is **IMP-046 DONE** (#72). Hybrid Step 5a DM-only send is **IMP-047 IN_PROGRESS**. Weekly review CLI is **IMP-048 BACKLOG**. Hybrid clock prompt canonical copies + server read-back is **IMP-049 BACKLOG**. Per-channel `send_enabled` config gate is **IMP-050 BACKLOG**. Publisher inventory / desk-bot removal / Chart–TradingView webhook ban / panel reconcile is **IMP-051 BACKLOG**. Chart OPTION A (`render_png` retirement; librarian files TV snapshots) is **IMP-056 BACKLOG** (behind Monday 2026-09-21 unattended Sydney Morning; docs only this PR).
...
- Principal 2026-09-20 FINAL chart scope is **IMP-056 BACKLOG** (OPTION A). Supersedes retrieval framing and option B. Chart is librarian, not renderer, not retriever. `render_png` stays until after Monday 2026-09-21 unattended Sydney Morning. This PR is docs/queue only. Paper only. No Telegram. No C-00x. Does not occupy the IMP-047 slot. IMP-051 Chart/TV webhook ban remains.
