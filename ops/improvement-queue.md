...
| **Dependencies** | IMP-020 DONE (#52). IMP-021 DONE (#53) — Ops holds send until config-backed. IMP-032 DONE (#57). Sister implementation [#59](https://github.com/ElChopa11/market-memory/pull/59) merged (`bc-3c465873`). |
| **Risk level** | Low–medium (language; publishing before config-backed). |
| **Status** | IN_PROGRESS |
| **PR** | Intake [#58](https://github.com/ElChopa11/market-memory/pull/58) — paper only; implementation [#59](https://github.com/ElChopa11/market-memory/pull/59) merged (sister `bc-3c465873`) |
| **Lesson learned** | *(fill at close)* |
...
| IMP-033 | Ops + Research | Ops/Don | IN_PROGRESS | Canonical watchlist monitor.yaml Principal lock — intake [#58](https://github.com/ElChopa11/market-memory/pull/58); implementation [#59](https://github.com/ElChopa11/market-memory/pull/59) merged. PATCH: SPCX→NASDAQ:SPCX `idio`; CBRS→NASDAQ:CBRS `semis_ai`; CHIPIUSD→HL:CHIP; VVVUSD→HL:VVV; PURR→HL:PURR. Unresolved until Principal paste: SAMSUN, KOSDA. FRED **ELIGIBLE** (not CLOSED). |

`IN_PROGRESS` count: **1**. IMP-000–IMP-021 and IMP-030–IMP-032 are `DONE`. IMP-033 is `IN_PROGRESS` (intake [#58](https://github.com/ElChopa11/market-memory/pull/58) — paper only; implementation [#59](https://github.com/ElChopa11/market-memory/pull/59) merged). IMP-022–IMP-029 are `BACKLOG` (source-evaluation 2026-09-18; no adapters). OPEN incidents: SCHED-001, BRIEF-TAG-20260918, SRC-STOOQ-404 (not closed). ELIGIBLE: SRC-FRED-MISSING-ENV (2026-09-19 `--no-db`; not CLOSED). object_store `missing_env` stays OPEN (not a SRC-* id). Single implementation thread.
