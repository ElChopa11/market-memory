...
| **Lesson learned** | Panel SCHEDULE cron is dead (7 routines, 0 fires). Panel WEBHOOK is dead (C1 silent). Two clocks work: Actions durable commit-back (`actions-b1-35727756341`) and Grok Bot app routines (`box-us-pre-20260922T133710Z`). Controls belong on a path that provably runs. Grok path depends on an awake agent and on Auto-review allowing the Shell call; the 2026-09-22 US Pre-Market fire correctly degraded to dry `--no-db` with DQ unavailable. Open question: is Auto-review configurable for this path, or permanently dry-only? |
...
**CLOSED — clock / invoker** (not OPEN; two clocks: Actions commit-back + Grok Bot app routines)

| ID | Desk | Owner | Status | Notes |
|---|---|---|---|---|
| SCHED-001 | Ops | Ops | CLOSED | run_id `actions-b1-35727756341`. hybrid-sydney-morning #1 Success ~21s; stamp `857f55c` on `main` (completions/ only). Two clocks: Actions commit-back (#90) and Grok Bot app routine `box-us-pre-20260922T133710Z`. Panel SCHEDULE and WEBHOOK dead. |
...
- **SCHED-001 CLOSED** 2026-09-22 citing run_id `actions-b1-35727756341` (hybrid-sydney-morning #1 Success ~21s; stamp commit `857f55c` on `main`, completions/ only). Same-day correction: two clocks, not Actions-only. Panel SCHEDULE cron dead (7 routines, 0 fires). Panel WEBHOOK dead (C1 silent). Grok Bot app routine works (run_id `box-us-pre-20260922T133710Z`; Auto-review blocked `--live`, degraded to dry `--no-db`, DQ unavailable — open Principal question). Actions remains the durable commit-back clock (#90). House lesson corrected. Forced-dispatch `late` is correct; next on-anchor is 06:30 Australia/Sydney.
