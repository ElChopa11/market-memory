...
| **Desk** | Ops + Research |
| **Owner** | Ops/Don |
| **Problem** | IMP-020 daily scan is DONE (#52). Versioned `config/watchlist/monitor.yaml` is now on `main` via sister [#59](https://github.com/ElChopa11/market-memory/pull/59) (merge `1403f57`). Remaining intake: first Intel scan pack (Ops/sole publisher holds Telegram until that pack); unresolved aliases SAMSUN, KOSDA. Membership still risks being misread as a call. |
| **Evidence** | Principal lock already in [`config/universe.yaml`](../config/universe.yaml): `in_universe` crypto BTC + equities NVDA AVGO MSFT META JPM XOM; `watch_only` crypto ETH UNI AAVE + equities SMH XLF; `deferred_must_cut` stays archived. IMP-020 DONE (#52); [ADR 0009](../ADR/0009-phase6c4-watchlist.md); [ADR 0014](../ADR/0014-canonical-watchlist-monitor.md) on `main`; [docs/runbooks/watchlist.md](../docs/runbooks/watchlist.md). Sister [#59](https://github.com/ElChopa11/market-memory/pull/59) **MERGED** to `main` (`1403f57`, 2026-09-19T00:14:23Z; branch `cursor/canonical-watchlist-monitor-5515`; `bc-3c465873`). [`config/watchlist/monitor.yaml`](../config/watchlist/monitor.yaml) is on `main`. YAML implementation DoD item satisfied. First scan unblocked for resolved names; SAMSUN/KOSDA still out. IMP-032 DONE (#57). Confirmed no other IMP-* `IN_PROGRESS` at intake. **Principal PATCH resolved (do not invent further tickers):** SPCX → NASDAQ:SPCX — Space Exploration Technologies Corp (SpaceX) — USD — NOT SPAC ETF / CAD DRC / memecoin — cluster `idio` (pending corr); CBRS → NASDAQ:CBRS — Cerebras Systems Inc — USD — IPO 2026-05-14 @ $185 — cluster `semis_ai` (MOVED from idio); CHIPIUSD → HL:CHIP (Hyperliquid coin CHIP; display CHIPIUSD); VVVUSD → HL:VVV; PURR → HL:PURR. **Still unresolved (out of ideas until Principal paste):** SAMSUN, KOSDA. |
| **Proposed outcome** | Versioned `config/watchlist/monitor.yaml` encoding the Principal lock with tiers/clusters for crypto then base, plus resolved SPCX/CBRS/CHIPIUSD/VVVUSD/PURR aliases and NEW_LISTING / LOCKUP WATCH rules. `lab watchlist scan` (or equivalent) reads this config. Membership is not a call. No universe expand. Do not invent unresolved aliases (SAMSUN, KOSDA). OPEN incidents untouched. Ops does not publish until config-backed. Sole publisher holds Telegram until the Intel scan pack. |
| **Definition of done** | Queue: IMP-032 DONE (#57). This item the only implementation thread. Plan [plans/IMP-033-watchlist-monitor-yaml.md](plans/IMP-033-watchlist-monitor-yaml.md). **YAML DoD satisfied:** `config/watchlist/monitor.yaml` + tiers/clusters **MERGED** on `main` via sister [#59](https://github.com/ElChopa11/market-memory/pull/59) (`1403f57`; branch `cursor/canonical-watchlist-monitor-5515`; `bc-3c465873`). First scan unblocked for resolved names; SAMSUN/KOSDA still out. Sole publisher holds until Intel scan pack. `lab watchlist scan` (or equivalent) config-backed. Tests. OPEN incidents unchanged (SCHED-001, BRIEF-TAG-20260918, SRC-STOOQ-404). SRC-FRED-MISSING-ENV is **ELIGIBLE** (not CLOSED) per Principal QUEUE CLOSURE RULE. No `live.yaml` / signing / credentials / delivery send-path edits. Do not reopen IMP-020. IMP-033 stays **IN_PROGRESS** on this intake until remaining hygiene lands (do not merge #58 here). **NEW_LISTING** (<200 daily bars): tag `NEW_LISTING` + `days_of_history`; SMA200 = `n/a (insufficient history: <n> bars)` never "?" / never silent shorter MA; route LISTINGS pod; post-IPO framework; lockup proximity + float/borrow/spread warnings; `UNTRADEABLE_AT_SIZE` observation-only. **LOCKUP WATCH:** EDGAR confirm for CBRS + SPCX — do not assume 180d. Lockup inside horizon = gate 5 blackout. Do not invent SAMSUN / KOSDA. |
...
| **Dependencies** | IMP-020 DONE (#52). IMP-021 DONE (#53) — Ops holds send until Intel scan pack. IMP-032 DONE (#57). Sister implementation [#59](https://github.com/ElChopa11/market-memory/pull/59) **MERGED** to `main` (`1403f57`; branch `cursor/canonical-watchlist-monitor-5515`; `bc-3c465873`). |
| **Risk level** | Low–medium (language; publishing before config-backed). |
| **Status** | IN_PROGRESS |
| **PR** | Intake [#58](https://github.com/ElChopa11/market-memory/pull/58) — paper only (do not merge this intake); implementation [#59](https://github.com/ElChopa11/market-memory/pull/59) **MERGED** (`1403f57`; branch `cursor/canonical-watchlist-monitor-5515`; sister `bc-3c465873`) |
| **Lesson learned** | *(fill at close)* |
...
| IMP-033 | Ops + Research | Ops/Don | IN_PROGRESS | Canonical watchlist monitor.yaml Principal lock — intake [#58](https://github.com/ElChopa11/market-memory/pull/58); sister [#59](https://github.com/ElChopa11/market-memory/pull/59) **MERGED** (`1403f57`; `config/watchlist/monitor.yaml` on `main`). YAML DoD satisfied. First scan unblocked for resolved names; SAMSUN/KOSDA still out. Sole publisher holds until Intel scan pack. PATCH: SPCX→NASDAQ:SPCX `idio`; CBRS→NASDAQ:CBRS `semis_ai`; CHIPIUSD→HL:CHIP; VVVUSD→HL:VVV; PURR→HL:PURR. FRED **ELIGIBLE** (not CLOSED). |

`IN_PROGRESS` count: **1**. IMP-000–IMP-021 and IMP-030–IMP-032 are `DONE`. IMP-033 is `IN_PROGRESS` (intake [#58](https://github.com/ElChopa11/market-memory/pull/58) — paper only, do not merge this intake; sister [#59](https://github.com/ElChopa11/market-memory/pull/59) **MERGED** — YAML on `main`). IMP-022–IMP-029 are `BACKLOG` (source-evaluation 2026-09-18; no adapters). OPEN incidents: SCHED-001, BRIEF-TAG-20260918, SRC-STOOQ-404 (not closed). ELIGIBLE: SRC-FRED-MISSING-ENV (2026-09-19 `--no-db`; not CLOSED). object_store `missing_env` stays OPEN (not a SRC-* id). Single implementation thread.
...
Phase 5 desk/delivery architecture is **IMP-009 DONE** (#40). Polygon equities + HL structure is **IMP-010 DONE** (#41). Quant factor library is **IMP-011 DONE** (#42). Desk runners are **IMP-012 DONE** (#43). Telegram delivery is **IMP-013 DONE** (#44). Phase 6a PG NOTIFY mesh is **IMP-014 DONE** (#45). Phase 6b flow+macro+regime is **IMP-015 DONE** (#46). Phase 6c PLAYBOOK + fan-out is **IMP-016 DONE** (#47). Phase 6c-1 five-desk roster is **IMP-018 DONE** (#49). Phase 6c-2 naming layer is **IMP-019 DONE** (#51). Phase 6c-4 watchlist monitor is **IMP-020 DONE** (#52). Phase 6c-5 delivery expansion is **IMP-021 DONE** (#53). Phase 6d listings/IPO is **IMP-017 DONE** (#54). Phase 6e scorecards + queue automation is **IMP-030 DONE** (#55). Phase 6f decay-watch is **IMP-031 DONE** (#56). Call-card vs Quant SoT language alignment is **IMP-032 DONE** (#57). Canonical watchlist `monitor.yaml` Principal lock is **IMP-033 IN_PROGRESS** (this intake PR [#58](https://github.com/ElChopa11/market-memory/pull/58); sister [#59](https://github.com/ElChopa11/market-memory/pull/59) **MERGED** — `config/watchlist/monitor.yaml` on `main`; do not reopen IMP-020).
...
- IMP-033 intakes canonical watchlist `monitor.yaml` (Principal lock + tiers/clusters). IMP-020 stays DONE (#52) — do not reopen. Paper only. No send. OPEN incidents untouched. Sister [#59](https://github.com/ElChopa11/market-memory/pull/59) **MERGED** to `main` (`1403f57`; branch `cursor/canonical-watchlist-monitor-5515`; `bc-3c465873`). YAML DoD satisfied. Single-threaded: IMP-033 is the only `IN_PROGRESS` item. Confirmed no other IMP-* `IN_PROGRESS` at intake.
- Principal PATCH on IMP-033 (intake #58, still `IN_PROGRESS`): resolved SPCX → NASDAQ:SPCX `idio` (pending corr); CBRS → NASDAQ:CBRS `semis_ai` (moved from idio); CHIPIUSD → HL:CHIP (display CHIPIUSD); VVVUSD → HL:VVV; PURR → HL:PURR. Unresolved until Principal paste: SAMSUN, KOSDA — do not invent. NEW_LISTING + LOCKUP WATCH (EDGAR confirm CBRS/SPCX; no 180d assumption; lockup inside horizon = gate 5 blackout). Sister [#59](https://github.com/ElChopa11/market-memory/pull/59) **MERGED** with the same patch. First scan unblocked for resolved names; SAMSUN/KOSDA still out. Sole publisher holds until Intel scan pack. OPEN incidents SCHED-001 / BRIEF-TAG-20260918 / SRC-STOOQ-404 untouched.
...
- Sister implementation [#59](https://github.com/ElChopa11/market-memory/pull/59) (branch `cursor/canonical-watchlist-monitor-5515`; `bc-3c465873`) **MERGED** to `main` (`1403f57`, 2026-09-19T00:14:23Z). Linked from IMP-033 Evidence / Status board / plan / #58 PR body. YAML on `main`. Do not merge intake #58.
