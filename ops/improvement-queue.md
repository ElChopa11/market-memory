...
|---|---|---|
| **P0** | Scheduler chain defects before Monday 06:30 AEST. Hive group stays frozen. SCHED-001 CLOSED (`actions-b1-35727756341`). | IMP-056 DONE (#85). IMP-047 DONE #73; IMP-046 DONE #72; IMP-042 DONE #68. Pack envelope/provenance on `--from-markdown` is IMP-057 BACKLOG. Retire `render_png` / CHART_ARTIFACT = TV ref + structured levels is IMP-058 BACKLOG. IMP-059 C-00x instance auto-track + detectors is PARKED (do not build). IMP-049/050/051 stay BACKLOG. |
...
### SCHED-001 — Sydney 08:00 digest never fired → CLOSED (Grok clock, Actions execution)
...
| **Desk** | Ops |
| **Owner** | Ops |
| **Problem** | Sydney 08:00 digest (successor: `grok.sydney_morning` 06:30 Australia/Sydney) was configured and never fired from the panel. Weekday windows existed. NY-cron sibling briefs completed. Panel SCHEDULE cron is dead (7 routines, 0 fires). Panel WEBHOOK is dead (C1 silent). |
| **Evidence** | **CLOSED** citing run_id `actions-b1-35727756341`. Job hybrid-sydney-morning #1 Success ~21s, `stage1-stamp` green. Stamp commit `857f55c` on `main` by `github-actions[bot]` (path scoped to `ops/reports/scheduler/completions/` only). Routine `grok.sydney_morning`; `scheduled_for` `2026-09-21T20:30:00Z`; actual `2026-09-22T12:32:21Z`; delta `57741s`; status `late` (correct for forced dispatch vs yesterday's anchor). Completion: [reports/scheduler/completions/grok.sydney_morning__20260921T203000Z.json](reports/scheduler/completions/grok.sydney_morning__20260921T203000Z.json). Grok pure-clock proof: run_id `box-us-pre-20260922T133710Z` (`ops/reports/scheduler/completions/grok.us_pre_market__20260922T130000Z.json`); Auto-review blocked `--live`; degrade-to-dry (`--no-db`, DQ unavailable) is permanent. Historical root cause: [reports/scheduler/2026-09-19-sched-001-root-cause.md](reports/scheduler/2026-09-19-sched-001-root-cause.md). Close pack: [reports/incident-closures/20260922-sched-001-actions-invoker-close.md](reports/incident-closures/20260922-sched-001-actions-invoker-close.md). House lesson: [config/knowledge/house-lessons.md](../config/knowledge/house-lessons.md) (2026-09-22 Principal decision). ADR: [ADR/0019-grok-clock-actions-execution.md](../ADR/0019-grok-clock-actions-execution.md). |
| **Proposed outcome** | Miss detector remains the standing control. Principal 2026-09-22: Grok routine is a pure clock plus agent wake (stamp + wake; no live fetch; no delivery). Actions is execution (fetch, brief, deliver; runner runs the CLI and commits output; B1 Stage 1 stamp is PR #90). The box is interactive desk work, not a production host. Panel SCHEDULE cron and panel WEBHOOK stay dead. Degrade-to-dry is permanent; no standing Auto-review allow. The Sydney workflow stays one AEST cron; every fire stamps. |
...
| **Lesson learned** | Panel SCHEDULE cron is dead (7 routines, 0 fires). Panel WEBHOOK is dead (C1 silent). Principal 2026-09-22: Grok is a pure clock plus agent wake (`box-us-pre-20260922T133710Z`); Actions is execution (`actions-b1-35727756341`); the box is interactive desk work, not a production host. Degrade-to-dry is permanent; no standing Auto-review allow. Follow-ons, not built: Neon/R2 credentials in Actions secrets; separate Actions Telegram bot; Stage 2 Principal-DM pack; minutes of 06:30 cron drift are accepted as drift, not as a skip. |
...
| **Desk** | Ops |
| **Owner** | Ops |
| **Problem** | Object store is a **DOWN SERVICE**. On the bot box, MinIO `:9000` is missing / connection refused — **expected**: `docker-compose.yml` is local-dev only (#88 house lesson); bot boxes are containers without a nested Docker host. This is **not** a missing credential. Deploy direction is **managed Postgres + S3-compatible store (Neon + Cloudflare R2 decided)**. A prepare-only change set (env names, Actions secret checklist, `lab migrate --sql`, R2 client glue) can land without credentials. The **live wire stays blocked** on Principal-supplied Actions secrets. `MINIO_*` defaults may still be present as local-dev leftovers. Alias: `SRC-object_store` / source id `object_store`. |
| **Evidence** | 2026-09-19 environment audit (Don) + [ops/reports/incident-closures/20260919-environment-audit-record-correction.md](reports/incident-closures/20260919-environment-audit-record-correction.md) (DOWN SERVICE, not missing_env). House lesson [config/knowledge/house-lessons.md](../config/knowledge/house-lessons.md) 2026-09-22 + [#88](https://github.com/ElChopa11/market-memory/pull/88): compose is not bot-box deploy; Neon+R2 decided, not wired. 2026-09-23 prepare packet [ops/reports/persistence/2026-09-23-neon-r2-prepare.md](reports/persistence/2026-09-23-neon-r2-prepare.md) and [docs/runbooks/persistence-neon-r2.md](../docs/runbooks/persistence-neon-r2.md). `hybrid-sydney-morning.yml` not modified. Source-health samples still show object_store unavailable / fail-closed. |
...
| **Definition of done** | Durable object store reachable on the Memory deploy path (not compose-on-box) and a down/parked backend is not filed as missing credentials. Stays OPEN while Neon/R2 are not reachable. Prepare-only docs and offline SQL are not a close. |
| **Non-goals** | Wiring Neon/R2 in a queue-hygiene PR; putting Neon/R2 credentials on the bot box; enabling Sydney-morning Postgres writes before secrets exist; treating compose/`.env.example` defaults as a credential incident; changing ingest; scheduler code; equity work. |
...
| **Status** | DONE |
| **PR** | [#85](https://github.com/ElChopa11/market-memory/pull/85) |
| **Lesson learned** | Merged to `main` (#85, 2026-09-20). Heartbeat table idempotent migrate; wrong-anchor writes no completion row; miss-check `--baseline-before today` labels known-missed. SCHED-001 stays OPEN. Phantom IN_PROGRESS cleared on hygiene PR. |
...
| **Definition of done** | Pack path requires envelope + provenance + gaps per 6c-3. Tests. `--no-send` default. Does not occupy an implementation slot while PARKED/BACKLOG. **Not built now.** |
| **Non-goals** | Blocking Monday 06:30 AEST. Telegram send. Changing Hive freeze. Occupying IN_PROGRESS. |
| **Dependencies** | IMP-016 / 6c-3 DONE (#47). IMP-056 DONE (#85). |
...
### IMP-058 — Retire stdlib render_png; CHART_ARTIFACT = TradingView ref + structured levels

| Field | Value |
|---|---|
| **ID** | IMP-058 |
| **Priority** | P2 |
| **Type** | Research / chart product |
| **Desk** | Research (chart sleeve) |
| **Owner** | Research / Ops |
| **Problem** | Principal 2026-09-20 Option 1: the stdlib `render_png` path is a **160×90 closes-only polyline with no axes/labels**. That is a thumbnail, not a chart. It cannot inform a decision; every playbook run that produces one adds an artifact the desk then explains. |
| **Evidence** | Principal 2026-09-20 Option 1 (queue-only this PR). Current encode is 160×90 in `packages/desks/src/mm_desks/chart.py` (`render_png` / `_encode_png`); playbook writes `png_by_hash` + `png_filename` in `packages/desks/src/mm_desks/playbook.py`. Naming still says “Levels + PNG”. Not Monday-critical. |
| **Proposed outcome** | Retire the stdlib PNG path. `CHART_ARTIFACT` becomes a **TradingView reference plus structured levels**: exchange-qualified instrument, timeframe, TV snapshot/URL, `as_of`, `content_hash`. Level set (`session_vwap`, `prior_high`, `prior_low`, and the rest of `ChartLevels`) lives on the artifact as **queryable fields**, not pixels. Chart Desk stays the Research sleeve. Zero Telegram from Chart. Don → `lab deliver` remains the sole exit. |
| **Definition of done** | **Later PR, not this one.** Remove `render_png` / `_encode_png` / `png_by_hash` and the playbook call site in `packages/desks/src/mm_desks/chart.py` + `playbook.py`. `CHART_ARTIFACT` fields: exchange-qualified instrument, timeframe, TV snapshot/URL, `as_of`, `content_hash`. Level set moves into the artifact as structured fields. Update 6c-3 ladder / desk-charters so `CHART_ARTIFACT` means reference-plus-levels. Chart stays Research sleeve; zero Telegram from Chart; Don → `lab deliver` sole exit. Paper only. No Telegram. No C-00x. |
| **Non-goals** | Implementing the retirement in this PR. Occupying IN_PROGRESS. Blocking Monday 2026-09-21 unattended Sydney Morning fire. Telegram send. Chart as a sixth desk. Chart/TradingView webhook as a publisher (IMP-051 ban stays). C-00x compute. Auto-merge. Gate waiver. |
| **Dependencies** | After Monday 2026-09-21 unattended fire. IMP-016 / 6c-3 DONE (#47). IMP-056 DONE (#85). IMP-051 TV webhook ban stays (TV ref ≠ Chart webhook publisher). |
| **Risk level** | Low this PR (queued only). Medium if left: playbook keeps producing a thumbnail we then explain. |
| **Status** | BACKLOG |
| **PR** | this PR (docs/queue only) |
| **Lesson learned** | *(queued — do not build now; land behind Monday 2026-09-21 unattended fire)* |


### IMP-059 — C-00x instance auto-track + detectors

| Field | Value |
|---|---|
| **ID** | IMP-059 |
| **Priority** | P2 |
| **Type** | Quant / candidate study infrastructure |
| **Desk** | Quant |
| **Owner** | Quant |
| **Problem** | Phase 6e pack-compare (#55) is not instance auto-track. C-001/C-002/C-003 stay `INTAKE_ONLY` until detectors exist and scorecards auto-track instances. L2 sprint P2 named the instance ledger. |
| **Evidence** | Principal 2026-09-22 queue hygiene. [research/candidates/README.md](../research/candidates/README.md) rule 9. IMP-040 DONE (#66) is necessary but not sufficient to unpark compute. |
| **Proposed outcome** | Instance auto-track + C-001/002/003 detectors; fixture/`--no-db`; per-instrument nulls; `n < n_min` → INSUFFICIENT SAMPLE; IC Gate 1 FAIL stands; no sizing; paper only. |
| **Definition of done** | Plan [plans/IMP-059-c00x-instance-autotrack-detectors.md](plans/IMP-059-c00x-instance-autotrack-detectors.md). Detectors + instance auto-track on fixture/`--no-db`; per-instrument nulls; `n < n_min` → INSUFFICIENT SAMPLE; IC Gate 1 FAIL intact; no sizing; cards stay `INTAKE_ONLY` until Principal unparks. **Not built now — PARKED.** |
| **Non-goals** | Neon/R2 wiring. Telegram. Detector implementation in this hygiene PR. Study compute. Inventing `config/reference/listings.yaml`. Reopening C-001/002/003. Clearing IC Gate 1 FAIL. Flipping cards off `INTAKE_ONLY`. Occupying IN_PROGRESS. |
| **Dependencies** | IMP-030 DONE (#55). IMP-039 READY (#61). IMP-040 DONE (#66). Principal unpark required before READY→IN_PROGRESS. |
| **Risk level** | Medium if built early (operators may treat detector output as a call or size). Low while PARKED. |
| **Status** | PARKED |
| **PR** | this PR (docs/queue only — plan file + PARKED) |
| **Lesson learned** | *(Principal 2026-09-22: stays PARKED; do not build detectors)* |
...
| IMP-056 | Ops | Ops | DONE | [#85](https://github.com/ElChopa11/market-memory/pull/85) Scheduler chain defects: heartbeat table, wrong-anchor no row, miss-check baseline. |
...
**CLOSED — clock / invoker** (not OPEN; Grok pure clock, Actions execution)

| ID | Desk | Owner | Status | Notes |
|---|---|---|---|---|
| SCHED-001 | Ops | Ops | CLOSED | run_id `actions-b1-35727756341`. hybrid-sydney-morning #1 Success ~21s; stamp `857f55c` on `main` (completions/ only). Principal 2026-09-22: Grok pure clock `box-us-pre-20260922T133710Z`; Actions execution (#90, [ADR 0019](../ADR/0019-grok-clock-actions-execution.md)). Panel SCHEDULE and WEBHOOK dead. |

**OPEN incidents — services / infrastructure** (not a missing-env credentials list)

| ID | Desk | Owner | Status | Notes |
|---|---|---|---|---|
| SRC-OBJECT-STORE | Ops | Ops | OPEN | **DOWN SERVICE**: MinIO missing on bot box (compose local-dev only, #88). Neon+R2 prepare-only (env names, `lab migrate --sql`); live wire blocked on Principal Actions secrets. Alias `SRC-object_store`. Do not close by setting `MINIO_*` or starting MinIO on-box. |
...
- **SCHED-001 CLOSED** 2026-09-22 citing run_id `actions-b1-35727756341` (hybrid-sydney-morning #1 Success ~21s; stamp commit `857f55c` on `main`, completions/ only). Principal decision the same day: Grok routine is a pure clock plus agent wake (run_id `box-us-pre-20260922T133710Z`; no live fetch; no delivery; degrade-to-dry is permanent; no standing Auto-review allow). Actions is execution (fetch, brief, deliver; this close is the Stage 1 stamp). The box is interactive desk work, not a production host. Panel SCHEDULE cron dead (7 routines, 0 fires). Panel WEBHOOK dead (C1 silent). Follow-ons recorded, not built: Neon/R2 in Actions secrets; separate Actions Telegram bot; Stage 2 Principal-DM pack; minutes of 06:30 cron drift are accepted as drift. The Sydney workflow stays one AEST cron; every fire stamps. Forced-dispatch `late` is correct; next on-anchor is 06:30 Australia/Sydney. ADR: [ADR/0019-grok-clock-actions-execution.md](../ADR/0019-grok-clock-actions-execution.md).
...
- Don queue hygiene 2026-09-22 (AEST): after batch #89/#91/#92/#93/#88 on `main` — **keep SRC-STOOQ-404 OPEN** (optional Stooq canary / monitoring; Pulse sole-path removed by #93 merge `b4db6e8` Polygon ETF proxies; VIX structural; cite [docs/runbooks/market-pulse.md](../docs/runbooks/market-pulse.md)). **SRC-OBJECT-STORE** status note refreshed: MinIO missing on bot box expected (compose local-dev; house lesson [#88](https://github.com/ElChopa11/market-memory/pull/88)); Neon+R2 decided but parked. IMP-056 DONE / IMP-059 PARKED / `IN_PROGRESS`=0 unchanged. SCHED-001 left alone (closed separately in #95 — do not conflict). Hold merge until Principal confirms. Paper only.
- Principal day order 2026-09-23 P1: Neon/R2 **prepare only**. Env names, Actions secret checklist, `lab migrate --sql`, R2 client glue (endpoint from env, `S3_REGION`, checksums `when_required`). No credentials. `hybrid-sydney-morning.yml` not modified. `SRC-OBJECT-STORE` stays OPEN. Live wire blocked on Principal Actions secrets. Runbook: [docs/runbooks/persistence-neon-r2.md](../docs/runbooks/persistence-neon-r2.md). Packet: [ops/reports/persistence/2026-09-23-neon-r2-prepare.md](reports/persistence/2026-09-23-neon-r2-prepare.md).
