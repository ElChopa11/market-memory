...
|---|---|---|
| **P0** | DM-only live path. Hive group stays frozen. SCHED-001 stays OPEN. | IMP-047 this thread (IMP-046 DONE #72; IMP-042 DONE #68 — do not touch miss detector) |
...
| **Definition of done** | DM-only CLI path. Group/desk `--send` remains SEND_FROZEN. Tests mock Telegram HTTP. PR documents on-box acceptance (Don). No miss-detector edits. Weekly authoring CLI not built (IMP-048 BACKLOG). Single IN_PROGRESS. |
| **Non-goals** | Lifting the Hive group freeze. Miss-detector / IMP-042 / miss-check edits. Weekly investment review authoring CLI. IMP-044 isolation. IMP-045 topics. Claiming live acceptance via mocks. Auto-merge. Gate waiver. |
