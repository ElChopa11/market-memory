...
-- Running upgrade 0010_unconditional_base_rates -> 0011_schedule_heartbeat
...
CREATE INDEX IF NOT EXISTS schedule_heartbeat_as_of_idx ON schedule_heartbeat (as_of_knowledge);

CREATE INDEX IF NOT EXISTS schedule_heartbeat_routine_idx ON schedule_heartbeat (routine_id);

UPDATE alembic_version SET version_num='0011_schedule_heartbeat' WHERE alembic_version.version_num = '0010_unconditional_base_rates';

-- Running upgrade 0011_schedule_heartbeat -> 0012_heartbeat_if_not_exists
...
CREATE TABLE IF NOT EXISTS schedule_heartbeat (
                ...
                payload_json JSONB NOT NULL DEFAULT '{}'::jsonb,
                created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
                CONSTRAINT schedule_heartbeat_status_check CHECK (status IN ('ok','late','missed','skipped','wrong_anchor')),
...
ALTER TABLE schedule_heartbeat DROP CONSTRAINT IF EXISTS schedule_heartbeat_status_check;

ALTER TABLE schedule_heartbeat ADD CONSTRAINT schedule_heartbeat_status_check CHECK (status IN ('ok','late','missed','skipped','wrong_anchor'));

UPDATE alembic_version SET version_num='0012_heartbeat_if_not_exists' WHERE alembic_version.version_num = '0011_schedule_heartbeat';
