...
| `status` | Timing class: `ok` / `late` / `wrong_anchor` (a fire). Not process exit. Unscheduled weekday is `wrong_anchor`, not `late`. |
...
3. Same `(routine_id, scheduled_anchor_ts)` key as the table. Higher-rank timing status wins (`ok` > `late` > `skipped` > `wrong_anchor` > `missed`).
