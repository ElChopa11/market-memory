...
`receipts/{routine_id}__{YYYYMMDDTHHMMSSZ}.deliver.json` is a Sydney Morning deliver receipt keyed on the stamp's `scheduled_anchor_ts` (not wall clock, not `run_id`). It is not a completion row. Miss-check does not read `receipts/`. Written only after deliver exits 0 and stdout JSON has `"sent": true`. Morning receipts include `capture_rows` (Neon read-back count for that capture, or null when the capture failed or was not rewritten). A drifted cron (or second trigger) arriving after a successful deliver for that anchor is a silent no-op / already_delivered, not a failure or miss. A failed send writes no receipt.
