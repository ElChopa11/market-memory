...
Default Hyperliquid **ingest** instruments: **BTC, ETH, UNI, AAVE perps** (Principal lock 2026-09-17 in `config/universe.yaml` — full membership kept). Equities on that file are a briefing / future equity-feed watchlist, not HL. Survivors are **not equal priority**: active calls (thesis work) are BTC, ETH, NVDA, AVGO, MSFT, META, JPM, XLF, XOM; UNI, AAVE, SMH, and IBIT are watch-only (no active call) after Skeptic PR #14 vs call cards PR #13 (IBIT added Principal 2026-09-17 as BTC equity proxy, BTC adjacency to RQ-20260917-A). Live trading is **hard-gated** until the Principal explicitly promotes after later-phase acceptance.
