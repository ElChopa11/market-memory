...
Default Hyperliquid **ingest** instruments: **BTC, ETH, UNI, AAVE perps** (Principal lock 2026-09-17 in `config/universe.yaml` — full membership kept). Equities on that file are a briefing / future equity-feed watchlist, not HL. Survivors are **not equal priority**: **in-universe** membership (thesis work; not a Quant verdict) is BTC, NVDA, AVGO, MSFT, META, JPM, XOM; ETH, UNI, AAVE, SMH, and XLF are **watch-only** (still ingested / still in membership; no thesis-priority) after Skeptic PR #14 vs call cards PR #13 and FAIL-patch PR #23. Live trading is **hard-gated** until the Principal explicitly promotes after later-phase acceptance.
