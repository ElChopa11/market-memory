...
Either DM secret absent → soft skip (stamp already succeeded; no brief; no POST). Both present → receipt check, then `lab brief close --live --no-db` then `lab deliver pack --from-markdown <brief> --to-principal-dm --i-mean-it --ignore-quiet-hours --no-db` only when this stamp's `scheduled_anchor_ts` has no deliver receipt. The receipt is written only after that command exits 0 and stdout JSON has `"sent": true`. A drifted cron (or second trigger) arriving after a successful deliver for that anchor is a silent no-op / already_delivered, not a failure or miss. Unset `TELEGRAM_CHAT_ID` does not block that DM POST. Any other preflight error does. Group `--send` stays SEND_FROZEN. Principal reviews before Telegram secrets are added; the schedule does not send until then. `POLYGON_API_KEY` and `FRED_API_KEY` are optional data richness (not required for stamp or DM). Absent Actions secrets resolve to empty; the brief marks those slots unavailable and the job does not fail. Principal adds Actions copies separate from the box `market-data/keys.env`. See [scheduler.md](scheduler.md).
