...
| CoinGecko | `timeout` / `http_5xx` / `rate_limited` | Always attempted on `--live`; unavailable note **kept** even when HL mid fills the table | Do not invent last/prior. Do not strip CG notes. |
