...
The approved layout is the dry-run in `ops/reports/renders/brief-template-dryrun.txt`. The comparison against the previous production format, from the same inputs, is `ops/reports/renders/brief-current-dryrun.txt`. Prices and health percentages move with the tape. The freeze is which sections, columns, and omissions are present.
