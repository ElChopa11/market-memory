...
- Health is a coverage rollup, not a pass/fail. A missing last is `unavailable` (weight 0), including structural VIX, so the line lists it and does not call it fresh. Polygon equities, USD, and oil at the expected prior session are not stale. An as-of older than that session is `stale` (weight 0) and the change cell is `no new session since`. FRED stays stale when its observation date matches the prior capture (`no new print since`). Crypto is not marked stale for a repeated as-of. Hyperliquid is the worst state among the states passed in; the close path passes all 14 perps. Weights stay fresh 1, degraded 0.5, stale 0, unavailable 0. The morning line lists only domains that are not fresh, at most two lines of 42 characters. When every scored domain is fresh the line is `Health 100%`.
- Funding is shown only when the hourly Hyperliquid print is off the interest baseline. The docs fix that baseline at 0.01% per 8 hours (`0.0001`), paid each hour at one eighth, so the hourly baseline is `0.0000125`. A print is on baseline when it is within half of `1e-6` of that rate (the recorded prints use 6 decimal places; `0.000013` is on baseline). On baseline it is omitted. It is not a gap. A missing funding print is a gap. Cite: Hyperliquid funding, interest component 0.01% / 8h.
...
- A change cell follows the observation as-of, not value equality. When ES, NQ, DXY, and CL are all the expected prior weekday, one header reads `EQUITY T-1 BY DESIGN (close YYYY-MM-DD)` and each line shows its value and the move versus the close before it. `no new session since` is only an as-of older than that session. The same FRED observation date prints `no new print since <date>`. A new as-of with an identical value prints `0.00%` or `+0.0bp`. BTC and ETH stay in the price table. SOL HYPE NEAR ARB UNI VVV ZEC DOGE XMR CHIP LTC PURR follow, from the same keyless `metaAndAssetCtxs` body. Each crypto line is mid and the 24h change versus `prevDayPx`. A missing value is a gap and is never printed as 0. A name with no print at all is the symbol on the gaps line.
- Funding is the hourly Hyperliquid rate, printed annualised, and only when it is off the baseline above. On baseline it is omitted, not a gap, even when a prior exists. A missing funding print is a gap. The `US Close` date is the shared equity bar date. When the T-1 header prints, that date is the same session. When Polygon is missing or returns no bars, the top line is `US Close unavailable` and is not the New York date of the fetch. US10Y is one line. Open interest is the percent change versus the previous stored capture, and only when that prior exists. No prior prints nothing, including nothing on the gaps line, and never a bare level. Basis is shown only with that prior. An empty liquidation list is unobserved and is omitted. Mid is the headline last and is not repeated.
