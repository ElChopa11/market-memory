...
- Funding is the hourly Hyperliquid rate, printed annualised, and only when it is off the baseline above. On baseline it is omitted, not a gap, even when a prior exists. A missing funding print is a gap. The `US Close` date and the T-1 header are the same session date. US10Y is one line. Open interest is the percent change versus the previous stored capture, and only when that prior exists. No prior prints nothing, including nothing on the gaps line, and never a bare level. Basis is shown only with that prior. An empty liquidation list is unobserved and is omitted. Mid is the headline last and is not repeated.
